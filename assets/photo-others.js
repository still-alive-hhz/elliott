(function () {
  var allItems = window.PHOTO_ITEMS || [];
  var items = allItems.filter(function (p) { return !!p.author; });
  ...
  var authorFilter = document.querySelector("#author-filter");
  ...
  var authors = dedup(items.map(function (p) { return p.author; }).filter(Boolean)).sort();

  authors.forEach(function (a) {
    var opt = document.createElement("option");
    opt.value = a;
    opt.textContent = a;
    authorFilter.append(opt);
  });
  ...
  function matchesPhoto(photo) {
    ...
    if (author && photo.author !== author) return false;
    return true;
  }
  ...
  [authorFilter, monthFilter, orderFilter].forEach(function (el) {
